...
if (!empty($_POST)) {
	...
	if ($fname != "" && $lname != "" && $username != "" && $password != "" && $location != "" && $contact != "") {
		...
		if ($login_ok) {
			...
			$query = "UPDATE user set firstname='$fname', lastname='$lname', password='$password', location='$location', contact='$contact' where username='$username'";
			...
			$response["success"] = 0;
			$response["message"] = "User-name mismatch";
			die(json_encode($response));
		}
